...
SUN_PENALTY         = 25.0
SLOW_DIST_MAX       = 40.0
EXPANSION_THRESHOLD = 6          # v8=10 → más agresivos temprano
WEAKEST_BONUS       = 15.0       # bonus por atacar al rival más débil
ENEMY_EXTRA         = 6          # buffer solo para planetas enemigos
...
# FIX 1 — required_ships sin buffer en neutrales
...
# FIX 2 — identificar al rival más débil
...
# FIX 3 — parámetros proporcionales al número de enemigos
def get_params(my_count, num_enemies=1):
    if num_enemies >= 3:
        return dict(min_defense=1, max_frac=0.95)
    if my_count < EXPANSION_THRESHOLD:
        return dict(min_defense=2, max_frac=0.92)
...
def choose_target(src, candidates, fleets, player, ang_vel, params, covered, expanding, weakest):
...
def agent(obs):
    ...
    expanding   = len(my_planets) < EXPANSION_THRESHOLD
    params      = get_params(len(my_planets), num_enemies)
    ...
    for src in my_planets:
        ...
        tgt, angle = choose_target(src, candidates, fleets, player,
                                   ang_vel, params, covered, expanding, weakest)
